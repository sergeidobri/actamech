...
export default async function ProceedingPage({
  ...
  const { volumes, title } = data;

  return (
    <ProceedingLayout title={title}>
      <MainContainer>
        <div className="flex flex-col gap-y-8 lg:flex-row">
          ...
          <div className="lg:pl-8 flex flex-wrap flex-row gap-8 w-full lg:border-l-border-primary lg:border-l-1 justify-start">
            {volumes.map((vol) => (
              <Fragment key={vol.id}>
                <ProceedingVolume
